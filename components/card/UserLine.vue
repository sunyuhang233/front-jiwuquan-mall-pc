      ...
      <el-popover placement="top" :offset="20" :width="320" trigger="hover">
        <template #reference>
          ...
          <el-badge :is-dot="!p.avatar || p.avatar === 'default.png'" class="item">
            <el-avatar :src="(!p.avatar || p.avatar === 'default.png') ? '' : (baseUrlImg + p.avatar)" class="hovers">
        ...
        <template #default>
          ...
          <div class="popup">
            <!-- 上传 -->
            <el-upload class="avatar-uploader" ref="uploader" style="width: 100%;height:100%;border-radius: 50%;" drag
              action="/api/user/info/avatar" :headers="{ 'Authorization': store.token }" method="PUT" :limit="1"
              :multiple="false" :auto-upload="true" :show-file-list="false" list-type="picture"
              :before-upload="beforeUpload" :on-success="updateSucess">
              <img w-6em h-6em v-if="avatarUrl" :src="baseUrlImg + avatarUrl" class="avatar" />
            ...
            <div class="bottom" flex-col>
              ...
              <div flex flex-wrap>
                <el-card shadow="hover" class="v-card" @click="toView('/my/info')">
                  <p class="icon myself" bg-yellow-5 i-solar:user-id-bold-duotone></p>
                  <p>主页</p>
                </el-card>
                <el-card shadow="hover" class="v-card" @click="toView('/shopcart')">
                  <p class="icon shopcart" bg-yellow-5 i-solar:cart-large-2-bold></p>
                  <p>购物车</p>
                </el-card>
                <el-card shadow="hover" class="v-card" @click="toView('/my/wallet')">
